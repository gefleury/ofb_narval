...
- `HF_TOKEN` : pour l'accès à Hugging Face. Nécessaire pour le modèle Llama3.1 (token à créer à partir d'un compte [Hugging Face](https://huggingface.co/))
...
## Usage
Narval utilise dans sa pipeline un modèle [Llama3.1-8b-Instruct](https://huggingface.co/meta-llama/Llama-3.1-8B-Instruct) de taille 16Go, téléchargé à partir d'[Hugging Face](https://huggingface.co/). Pour garder des temps de calcul raisonnables, il est fortement recommandé d'exécuter la pipeline complète sur une machine disposant d'un GPU $\gtrsim$ 16Go. En revanche, le développement du code et l'analyse des résultats peut être fait en local sur un PC de bureau.
...
- [Hugging Face](https://huggingface.co/) : pour l'accès aux modèles de langage [Flan-T5](https://huggingface.co/google/flan-t5-xl) (Google) et [Llama3.1-8b](https://huggingface.co/meta-llama/Llama-3.1-8B-Instruct) (Meta)
...
Les modèles de langage Flan-t5 et Llama3.1-8b utilisés dans NARVAL sont distribués respectivement sous licence [Apache2.0](https://choosealicense.com/licenses/apache-2.0/) et [Llama3.1](https://www.llama.com/llama3_1/license/). 
